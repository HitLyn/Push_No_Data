...
def mppi_push(pos_x = 0.3, pos_y = 0.7, theta = 30):
    ...
    target_action_trajectory_marker_pub  = rospy.Publisher('target_action_trajectory_marker', Marker, queue_size = 1)
    ...
    for step in range(STEP_LIMIT):
        ...
        target_action_trajectory, target_action = mppi.compute_noise_action() # np.array[T, 2]
        print('target shape verify : ', target_action_trajectory.shape, target_action.shape)

        # ###### target_action_list visualization ############
        target_action_trajectory_marker = set_target_action_marker(target_action_trajectory, target = True)

        target_action_trajectory_marker_pub.publish(target_action_trajectory_marker)
