...
def stack_time_step_input(env_time_step, state, timestep, episode, i):
     ...
     for step in range(timestep):
         idx = episode * (env_time_step - 1) + i + step
...
def stack_time_step_output(env_time_step, state, timestep, episode, i):
     ...
     idx = episode * (env_time_step - 1) + i + timestep - 1

     return state[idx]

class Model():
    def __init__(self, object_state_dim, robot_action_dim, env_time_step, rnn_units, batch_size, time_steps):
        """
        Args:
        ...
        self.model = tf.keras.Sequential([
            # tf.keras.layers.LSTM(rnn_units , return_sequences = True, recurrent_initializer='glorot_uniform', recurrent_regularizer=tf.keras.regularizers.l2(0.1), input_shape = [self.time_steps, 4], dropout = 0.8),
            # # tf.keras.layers.Dropout(0.8),
            tf.keras.layers.LSTM(rnn_units, recurrent_initializer='glorot_uniform', recurrent_regularizer=tf.keras.regularizers.l2(0.05), dropout = 0.8, input_shape = [self.time_steps, 4]),
    ...
    def LSTM_evaluate_step(self, input, target):
        predictions = self.model(input)
        loss = self.loss(target, predictions)
    ...
    def train(self, data_set, test_data_set, evaluate_data_set):
        """
        Args:
        ...
        for epoch in range(self.epochs):
            ...
            template = 'Epoch {}, Loss: {}, Evaluate loss: {}, Test Loss: {}'
    ...
    def data_preprocess(self, object_data, robot_data):
        ...
        sequences_num_per_episode = self.env_time_step - self.time_steps # how many sequences to generate per episode
        ...
        state_inputs = np.zeros((episode_num * (self.env_time_step - 1), 4))
        ...
        # self.mean = mean = state_inputs.mean()
        # self.std = std = state_inputs.std()
...
def main():
    ...
    model.train(data_set, test_data_set, evaluate_data_set)
