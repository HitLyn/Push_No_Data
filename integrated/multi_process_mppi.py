    ...
    def rollout(self, episode):
        for episode in range(episode):
            ...
            for step in np.arange(self.time_limit):
                ...
                current_state = [self.trajectory.get_relative_state(), self.trajectory.get_absolute_state(), self.trajectory.get_obstacle_position(), self.trajectory.get_goal_position(), step]
                # compute costs for K sample in multiprocess
                paths = self.mppi_compute_cost_parallel(self.predictor_args, current_state, rollout_num_per_cpu, self.U, step) # list of K elements, each element is a dict{observation: (self.T, dim_obs), actions: (self.T, 2), costs: (self.T,)}
                # get cost from paths
